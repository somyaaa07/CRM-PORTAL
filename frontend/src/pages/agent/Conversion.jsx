...
export default function Conversions() {
  ...
  const fetchStats = async () => {
    try {
      ...
      const lost = all.filter((l) =>
        ["Lost", "Not Interested"].includes(l.status),
      ).length;
      ...
      TABS.forEach((t) => {
        tabCounts[t.key] =
          t.key === "all"
            ? all.length
            : all.filter((l) => l.status === t.key).length;
      });
  ...
  useEffect(() => {
    if (urlStatus) setActiveTab(urlStatus);
  }, [urlStatus]);
        ...
        >
          ...
          >
            {stats.converted > 0 && (
              <div
                style={{
                  width: `${(stats.converted / stats.total) * 100}%`,
                  background: "#22C55E",
            ...
            {stats.lost > 0 && (
              <div
                style={{
                  width: `${(stats.lost / stats.total) * 100}%`,
                  background: "#EF4444",
                  transition: "width .5s ease",
                }}
              />
