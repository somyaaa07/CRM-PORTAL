...
const STATUS_META = {
  ...
  'Contacted':      { bg: '#EEEDFE', border: '#CECBF6', dot: '#7F77DD', text: '#3C3489' },
  'Follow-Up':      { bg: '#FAEEDA', border: '#FAC775', dot: '#BA7517', text: '#854F0B' },
  'Interested':     { bg: '#E1F5EE', border: '#9FE1CB', dot: '#1D9E75', text: '#085041' },
  'Not Interested': { bg: '#FAECE7', border: '#F5C4B3', dot: '#D85A30', text: '#993C1D' },
};

const AVATAR_COLORS = ['#378ADD', '#7F77DD', '#BA7517', '#1D9E75', '#D85A30', '#D4537E'];
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 11 }}>
        <div style={{
          ...
          fontSize: 13.5, color: ac, flexShrink: 0,
        }}>
          {initials(lead.name)}
        </div>
     <div
...
>
  ...
  <FiChevronRight style={{ color: "#94A3B8", flexShrink: 0, fontSize: 14 }} />
</div>
<p style={{ margin: 0, fontSize: 12, color: '#888780', marginTop: 2 }}>{lead.city}</p>
      ...
      <div style={{
        ...
        background: '#F7F5F0', borderRadius: 8, padding: '8px 11px', border: '1px solid #E8E6DF',
      }}>
        ...
        <span style={{ fontSize: 13, color: '#444441', fontFamily: "'DM Sans', sans-serif" }}>{lead.phone}</span>
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <button
          ...
          style={{
            ...
            fontFamily: "'Sora', sans-serif", cursor: 'pointer', transition: 'all 0.15s ease',
...
function StatPill({ status, count }) {
  ...
  return (
    <div style={{
      ...
      borderRadius: 10, padding: '10px 16px', textAlign: 'center', minWidth: 190,
    }}>
      <p style={{ margin: 0, fontSize: 20, fontWeight: 700, color: m.dot, fontFamily: "'Sora', sans-serif" }}>{count}</p>
      <p style={{ margin: 0, fontSize: 10.5, color: m.text, marginTop: 2, fontFamily: "'DM Sans', sans-serif" }}>{status}</p>
    </div>
  );
}
...
export default function MyLeads() {
  ...
  return (
    <>
      ...
      <div style={{ minHeight: '100vh', background: '#fefafa', padding: '28px 24px', fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
          ...
          <div>
            <h1 style={{ margin: 0, fontFamily: "'Sora', sans-serif", fontWeight: 700, fontSize: 21, color: '#2C2C2A' }}>
              My Leads
            </h1>
            <p style={{ margin: 0, fontSize: 12.5, color: '#888780', marginTop: 1 }}>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 22 }}>
          {Object.keys(STATUS_META).map(s => <StatPill key={s} status={s} count={counts[s] || 0} />)}
        </div>
        ...
        <div style={{ position: 'relative', marginBottom: 14 }}>
          ...
          <input
            ...
            style={{
              width: '100%', background: '#ffffff', border: '1px solid #D3D1C7', borderRadius: 11,
              padding: '10px 36px 10px 38px', fontSize: 13.5, color: '#2C2C2A', outline: 'none',
          ...
          {searchInput && (
            <button onClick={() => setSearchInput('')} style={{
              ...
              background: '#F1EFE8', border: '1px solid #D3D1C7', borderRadius: '50%',
              width: 20, height: 20, cursor: 'pointer', color: '#888780', fontSize: 11,
              display: 'flex', alignItems: 'center', justifyContent: 'center',
            }}>✕</button>
          )}
        </div>
          ...
          {STATUS_FILTERS.map(f => {
            ...
            return (
              <button key={f} onClick={() => setActiveFilter(f)} style={{
                ...
                fontWeight: isA ? 600 : 400, fontFamily: "'Sora', sans-serif",
                cursor: 'pointer', transition: 'all 0.15s ease',
                display: 'flex', alignItems: 'center', gap: 6,
              }}>
                {m && <span style={{ width: 6, height: 6, borderRadius: '50%', background: isA ? m.dot : '#B4B2A9', display: 'inline-block' }} />}
                {f}
                {isA && pagination && (
                  <span style={{
                    background: m ? m.border + '88' : '#D3D1C7', color: m ? m.text : '#444441',
                    borderRadius: 6, padding: '1px 6px', fontSize: 10.5,
                  }}>{pagination.totalLeads}</span>
                )}
              </button>
            );
          })}
        ...
        {activeFilter !== 'All' && (
          <div style={{
            ...
            <span>⬆️</span>
            <span><strong>"{activeFilter}"</strong> leads are sorted at the top</span>
            <button
              onClick={() => setActiveFilter('All')}
              style={{
                marginLeft: 'auto', background: 'none', border: 'none',
                color: '#378ADD', fontSize: 12, cursor: 'pointer',
        ...
        {loading ? (
          <div style={{ textAlign: 'center', paddingTop: 80 }}>
            <div style={{
              width: 36, height: 36, border: '3px solid #E1F5EE', borderTopColor: '#1D9E75',
              borderRadius: '50%', margin: '0 auto 14px', animation: 'spin 0.75s linear infinite',
            }} />
          ...
          <div style={{ textAlign: 'center', paddingTop: 80 }}>
            ...
            <p style={{ color: '#888780', fontSize: 14, marginBottom: 12 }}>No leads found</p>
            {(searchInput || activeFilter !== 'All') && (
              <button onClick={() => { setSearchInput(''); setActiveFilter('All'); }} style={{
          ...
          <>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 12 }}>
              {leads.map((lead, i) => (
                <div key={lead.id} style={{ animation: 'fadeUp 0.25s ease both', animationDelay: `${i * 35}ms` }}>
                  <LeadCard
                    lead={lead}
                    onCallClick={setSelectedLead}
                    onNavigate={(id) => navigate(`/leads/${id}`)}
                    isHighlighted={activeFilter !== 'All' && lead.status === activeFilter}
                  />
                </div>
              ))}
            </div>
