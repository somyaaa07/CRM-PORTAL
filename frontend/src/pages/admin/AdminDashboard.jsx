...
/* ─────────────────────────────────────────
   FONTS & GLOBAL STYLES
   Title / stat values  → Mono Rope (Google: Roboto Mono as fallback)
   Body / labels        → Jost
───────────────────────────────────────── */
const FontStyle = () => (
  <style>{`
    ...
    /* Mono Rope isn't on Google Fonts; we load Roboto Mono as the mono serif stand-in.
       If you have Mono Rope locally, add it via @font-face above and replace
       'Roboto Mono' with 'Mono Rope' throughout. */
    ...
    * {
    ...
    /* stat value uses mono title font */
    .stat-val {
...
const StatCard = ({ Icon, label, value, theme, onClick }) => (
  ...
  >
    ...
    <div
      className="chip"
      style={{ width: 40, height: 40, borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 16 }}
    >
...
const QuickLink = ({ to, Icon, title, desc, iconBg, iconColor }) => (
  ...
  >
    <div
      style={{ width: 40, height: 40, borderRadius: 12, background: iconBg, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}
    >
      <Icon size={17} color={iconColor} strokeWidth={2.1} />
    </div>
    <div style={{ flex: 1, minWidth: 0 }}>
      <p style={{ fontSize: '0.9rem', fontWeight: 600, color: '#1f2937', margin: 0 }}>{title}</p>
      <p style={{ fontSize: 12, color: '#9ca3af', marginTop: 2, margin: '2px 0 0' }}>{desc}</p>
    </div>
...
export default function AdminDashboard() {
  ...
  if (loading) return (
    <>
      <FontStyle />
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '8rem 0', color: '#c4b5fd', fontSize: 14 }}>
        Loading dashboard…
      </div>
    </>
  );
    ...
    <>
      ...
      <div style={{ minHeight: '100vh', background: '#f5f3ff', padding: '32px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 32 }}>
          <div>
            <h1 style={{ fontFamily: 'var(--font-title)', fontWeight: 700, fontSize: '1.5rem', color: '#1e1b4b', margin: 0, letterSpacing: '-0.5px' }}>
              Admin Dashboard
            </h1>
            <p style={{ fontFamily: 'var(--font-body)', fontSize: 13, color: '#8369cf', marginTop: 4, fontWeight: 400 }}>
              CRM overview · Live data
            </p>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#fff', border: '1.5px solid #ede9fe', borderRadius: 12, padding: '8px 16px', fontSize: 13, color: 'black', fontFamily: 'var(--font-body)', fontWeight: 500 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 20 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            {/* Conversion Rate */}
            <div style={{ background: 'rgba(255,255,255,0.85)', backdropFilter: 'blur(8px)', border: '1.5px solid #ede9fe', borderRadius: 20, padding: 24 }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 20 }}>
                <div>
                  <p style={{ fontFamily: 'var(--font-body)', fontWeight: 600, fontSize: '0.95rem', color: '#111827', margin: 0, display: 'flex', alignItems: 'center', gap: 8 }}>
                    <TrendingUp size={15} style={{ color: '#7c3aed' }} />
                    Overall Conversion Rate
                  </p>
                  <p style={{ fontFamily: 'var(--font-body)', fontSize: 12, color: '#9ca3af', marginTop: 3 }}>
                    Converted ÷ Total leads
                  </p>
                </div>
                <span style={{ fontFamily: 'var(--font-title)', fontSize: '2rem', fontWeight: 700, color: '#7c3aed', letterSpacing: '-1px', lineHeight: 1 }}>
                  {convRate}%
                </span>
              </div>
            ...
            {/* Chart */}
            <div style={{ background: 'rgba(255,255,255,0.85)', backdropFilter: 'blur(8px)', border: '1.5px solid #ede9fe', borderRadius: 20, padding: 24, flex: 1 }}>
              <p style={{ fontFamily: 'var(--font-body)', fontWeight: 600, fontSize: '0.95rem', color: '#111827', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
