...
import {
  ...
  Users, Upload, Bell, LogOut,
  CheckCircle2, ChevronRight,
  PanelLeftClose, PanelLeftOpen, PlusCircle,
} from 'lucide-react';
...
export default function Navbar() {
  ...
  // ── Total badge count ──────────────────────────────────
  const totalBadge =
    (user?.role === 'agent' ? alertStats.total : 0) +
    unreadCount;

  const agentLinks = [
    { to: '/agent/dashboard',    label: 'Dashboard',    Icon: LayoutDashboard },
    { to: '/agent/my-leads',     label: 'My Leads',     Icon: ClipboardList   },
    { to: '/agent/add-lead',     label: 'Add Lead',     Icon: PlusCircle      },
    ...
    <>
      <style>{`
        ...
        .logo-icon {
          ...
          box-shadow: 0 4px 12px rgba(124,77,255,0.35); flex-shrink: 0;
        }
        .logo-text {
          font-size: 0.92rem; font-weight: 800;
          color: ${ink}; letter-spacing: -0.02em;
          transition: opacity 0.15s, width 0.22s; overflow: hidden;
        }
        ...
        .collapse-btn {
          ...
          border: 1px solid rgba(11,7,21,0.09); background: transparent;
          display: flex; align-items: center; justify-content: center;
          cursor: pointer; color: rgba(11,7,21,0.35); flex-shrink: 0;
          transition: background 0.15s, color 0.15s;
        }
        .collapse-btn:hover { background: rgba(11,7,21,0.05); color: ${ink}; }
        ...
        .expand-btn {
          ...
          color: rgba(11,7,21,0.35); transition: background 0.15s, color 0.15s;
        }
        .sidebar.collapsed .expand-btn { display: flex; }
        .expand-btn:hover {
          background: rgba(124,77,255,0.08); color: ${purple};
        ...
        .nav-section-label {
          ...
          color: rgba(11,7,21,0.25); text-transform: uppercase;
        ...
        .nav-link {
          ...
          color: rgba(11,7,21,0.45); text-decoration: none;
          transition: background 0.15s, color 0.15s;
          white-space: nowrap; overflow: hidden; position: relative;
        }
        .nav-link:hover { background: rgba(11,7,21,0.04); color: ${ink}; }
        .nav-link.active { background: rgba(124,77,255,0.1); color: ${purple}; }
        ...
        .nav-link.active::before {
          ...
          background: ${purple}; border-radius: 0 3px 3px 0;
        }
        .sidebar.collapsed .nav-link { justify-content: center; padding: 9px 0; }

        .sidebar-footer {
          padding: 10px; border-top: 1px solid rgba(11,7,21,0.07);
        ...
        .bell-btn {
          ...
          color: rgba(11,7,21,0.45); font-family: ${font};
          font-size: 0.8rem; font-weight: 600;
          transition: background 0.15s, color 0.15s;
          white-space: nowrap; overflow: hidden; position: relative;
        }
        .bell-btn:hover { background: rgba(11,7,21,0.04); color: ${ink}; }
        .bell-btn.has-alert {
          color: ${purple}; background: rgba(124,77,255,0.06);
        }
        .sidebar.collapsed .bell-btn { justify-content: center; padding: 9px 0; }

        .bell-icon-wrap { position: relative; flex-shrink: 0; }
        .bell-badge {
          position: absolute; top: -4px; right: -4px;
          width: 16px; height: 16px; border-radius: 50%;
          font-size: 0.5rem; font-weight: 800; font-family: ${font};
        ...
        .bell-badge.notif    { background: ${purple}; }

        .bell-btn-label {
          overflow: hidden; transition: opacity 0.15s, max-width 0.22s;
        }
        ...
        .bell-dropdown {
          ...
          border: 1px solid rgba(11,7,21,0.09); border-radius: 14px;
          box-shadow: 0 16px 48px rgba(11,7,21,0.13);
          z-index: 200; overflow: hidden; max-height: 480px;
          overflow-y: auto;
        }
        .sidebar.expanded .bell-dropdown {
          left: 0; bottom: calc(100% + 8px); width: 100%;
        }
        ...
        .dropdown-title {
          font-size: 0.78rem; font-weight: 700; color: ${ink};
          display: flex; align-items: center; gap: 6px; font-family: ${font};
        }
        .dropdown-count {
          font-size: 0.65rem; font-weight: 700;
          padding: 2px 8px; border-radius: 20px; font-family: ${font};
        }
        .dropdown-count.overdue  { background: #fee2e2; color: #dc2626; }
        .dropdown-count.upcoming { background: #ffedd5; color: #ea580c; }
        .dropdown-count.notif    { background: rgba(124,77,255,0.1); color: ${purple}; }
        ...
        .alert-item.unread   { background: rgba(124,77,255,0.04); }
        .alert-item:hover    { background: rgba(124,77,255,0.06) !important; }
        ...
        .alert-dot.meta     { background: ${purple}; }
        .alert-dot.read     { background: #d1d5db; }
        ...
        .alert-tag.meta     {
          background: rgba(124,77,255,0.1); color: ${purple};
        }
        ...
        .user-pill {
          ...
          padding: 7px 10px;
          border: 1px solid rgba(11,7,21,0.08); border-radius: 11px;
          background: rgba(11,7,21,0.02); overflow: hidden;
        }
        .sidebar.collapsed .user-pill {
          padding: 7px 0; justify-content: center;
        ...
        .logout-btn {
          ...
          font-family: ${font}; font-size: 0.8rem; font-weight: 600;
        ...
        .mobile-bar {
          ...
          background: #fff;
          border-top: 1px solid rgba(11,7,21,0.07);
          z-index: 110; padding: 0 6px;
          align-items: center; justify-content: space-around; gap: 2px;
        }
        .mobile-link {
          flex: 1; display: flex; flex-direction: column;
          align-items: center; justify-content: center; gap: 3px;
          padding: 6px 4px; border-radius: 9px; text-decoration: none;
      ...
      <nav className={`sidebar ${collapsed ? 'collapsed' : 'expanded'}`}>
        ...
        <div className="sidebar-footer">
          ...
          {/* Bell — Agent + Admin dono ke liye */}
          <div className="bell-row">
            <button
              className={`bell-btn${totalBadge > 0 ? ' has-alert' : ''}`}
              ...
              <span className="bell-icon-wrap">
                ...
                {totalBadge > 0 && (
                  <span className={`bell-badge ${
                    alertStats.overdue > 0 ? 'overdue' :
                    unreadCount > 0        ? 'notif'   : 'upcoming'
                  }`}>
                    {totalBadge > 9 ? '9+' : totalBadge}
                  </span>
                )}
              </span>
              <span className="bell-btn-label">Notifications</span>
            ...
            {showDropdown && (
              <>
                ...
                <div className="bell-dropdown">
                  ...
                  {/* ── Follow-up Alerts (Agent only) ── */}
                  {user?.role === 'agent' && alerts.length > 0 && (
                    <>
                      <div className="section-divider">
                        <span>⏰ Follow-up Alerts</span>
                        <span style={{ color: alertStats.overdue > 0 ? '#dc2626' : '#ea580c' }}>
                          {alertStats.total}
                        </span>
                      </div>
                    ...
                    <>
                      ...
                      <div className="alert-list">
                       {notifications.filter(n => !n.isRead).slice(0, 5).map((notif) => (
                          <div
                            key={`notif-${notif.id}`}
                            className={`alert-item ${!notif.isRead ? 'unread' : ''}`}
                            onClick={() => {
                              markAsRead(notif.id);
                              if (notif.leadId) {
                          ...
                          >
                            <div className={`alert-dot ${
                              notif.type === 'meta-lead' ? 'meta' :
                              notif.isRead ? 'read' : 'meta'
                            }`} />
                            <div className="alert-info">
                            ...
                            {!notif.isRead && (
                              <span className="alert-tag meta">New</span>
                            )}
                          </div>
                        ))}
                      </div>
                  ...
                  {alerts.length === 0 && notifications.length === 0 && (
                    <div className="dropdown-empty">
                      ...
                      <span className="dropdown-empty-text">
      ...
      <nav className="mobile-bar">
        ...
        {/* Bell mobile */}
        <button
          className={`mobile-link${totalBadge > 0 ? ' active' : ''}`}
          style={{
            border: 'none', background: 'transparent',
            cursor: 'pointer', position: 'relative',
          }}
          onClick={() => navigate(
            user?.role === 'admin' ? '/admin/leads' : '/agent/my-leads'
          )}
        >
          <span style={{ position: 'relative', display: 'inline-flex' }}>
            ...
            {totalBadge > 0 && (
              <span style={{
                ...
                background: alertStats.overdue > 0
                  ? '#dc2626'
                  : unreadCount > 0
                  ? purple
                  : '#ea580c',
